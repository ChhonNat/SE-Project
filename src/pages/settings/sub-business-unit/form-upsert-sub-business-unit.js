...
const UpsertSubBusinessUnitForm = (props) => {

    const { openModal, onCloseModal, handleEventSuccessed, title, editData } = props;
    const { register, handleSubmit, reset, setValue, watch, formState, clearErrors } = useForm({ resolver: zodResolver(SubBusinessUnitModel) });
    ...
    const submit = async (data) => {
        ...
        Object.keys(data).forEach((key) => {
            ...
            if (KEY_POST.subBusinessUnit.includes(key)) {
                postData[key] = data[key];
            }
        });

        await sendRequest(!editData?.id ? API_URL.subBusinessUnit.create : API_URL.subBusinessUnit.edit + editData?.id, !editData?.id ? HTTP_METHODS.post : HTTP_METHODS.put, postData);
    }
...
export default UpsertSubBusinessUnitForm;