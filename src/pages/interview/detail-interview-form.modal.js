...
const InterviewFormDetailModal = (props) => {
  ...
  const mapKeyStringToView = {
    candidateName: { label: "Candidate" },
    positionName: { label: "Position" },
    positionLevel: { label: "Position Level" },
    departmentName: { label: "Department" },
    headDepartmentName: { label: "Head Department" },
    businessUnitName: { label: "Business Unit" },
    invitedAt: { label: "Invited At", type: "date" },
    invitedBy: { label: "Invited By" },
    interviewDate: {
      label: "Interview At",
      type: "date",
      dateFormat: "MMM DD, YYYY hh:mm A",
    },
    interviewResult: { label: "Interview Result" },
    interviewProcess: { label: "Interview Process" },
    status: { label: "Status" },
    remark: { label: "Remark" },
  };

  const mapKeyArrayToView = {
    committees: { label: "Interviewer Committees" },
    evaluationDetails: { label: "Evaluated History" },
  };
  ...
  const getCandidateDetail = useCallback(async () => {
    await sendRequest(
      API_URL.interview.detail + interview?.id,
      HTTP_METHODS.post
    );
          ...
          <>
            ...
            <DialogContent dividers>
              <Box sx={{ width: "100%" }}>
                ...
                >
                  {keyDisplays.map((key, index) => {
                    if (
                      mapKeyStringToView[key] &&
                        ...
                        >
                          ...
                          <label>
                            {mapKeyStringToView[key]?.type === "date"
                              ? moment(interviewDetail[key]).format(
                                  mapKeyStringToView[key]?.dateFormat
                                    ? mapKeyStringToView[key]?.dateFormat
                            ...
                            interviewDetail[key].map((intDetail) => (
                              <>
                                ...
                                >
                                  ...
                                  <label>{intDetail?.interviewResult}</label>
                                </Grid>
                                <Grid
                                  item
                                  paddingBottom={1}
                                ...
                                >
                                  ...
                                  <label>
                                    {moment(intDetail?.evaluatedAt).format(
                                      "MMM DD, YYYY hh:mm A"
                                    )}
                                  </label>
                                </Grid>
