...
const HomeAssessment = () => {
  ...
  const [openReferenceResultModal, setOpenReferenceResultModal] =
    useState(false);
  const [openReferenceDetailModal, setOpenReferenceDetailModal] =
    useState(false);
    ...
    <>
          ...
          moreOption: {
            buttons: [
              ...
              },
              {
                ...
                hidden: !user?.roles
                  ? false
                  : [ROLE.ROLE_TA_TEAM, ROLE.ROLE_TA_ADMIN].some((role) =>
      ...
      {openFileFormModal && (
        <ViewFileModal
          ...
          id={
            typeModal === "viewCVFile"
              ? editReference?.candidate?.id
              : editReference?.id
          }
          downloadFileUrl={
            typeModal === "viewCVFile"
              ? null
              : API_URL.referenceCheck.downloadRefForm
          }
